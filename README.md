...
默认测试覆盖纯数值契约、归档边界、媒体解码命令、输出事务、适配器的本地资产约束与假依赖端到端流程。真实归档审计和真实问题 1 smoke 均为 opt-in 测试：只有显式提供 `E_MOSEI_ARCHIVE`、`E_MOSEI_7ZA` 或 `E_MOSEI_Q1_CONFIG` 后才会执行；未配置时跳过是预期行为，不代表真实模型链路已在当前机器验证。
